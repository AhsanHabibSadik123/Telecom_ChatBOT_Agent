...
def build_retriever(question: str, k_faq: int = 3, k_tickets: int = 3, k_guides: int = 3) -> str:
    ...
    #     )
    # return RunnableLambda(retrieve("Internet not working"))

    return(
        faq_retriever.invoke(question)
        + tickets_retriever.invoke(question)
        + guides_retriever.invoke(question)
    )
        

if __name__ == "__main__":
    retrieve = build_retriever(question="Internet not working")
    print(retrieve)